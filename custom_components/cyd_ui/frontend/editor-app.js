export function startCydUiEditor(root,hass){
const document={
  querySelector:(selector)=>root.querySelector(selector),
  createElement:(tag)=>window.document.createElement(tag),
  get visibilityState(){return window.document.visibilityState;}
};
const state={ui:null,backendMap:null,catalog:null,icons:[],entities:[],ldrVoltage:null,deviceBrightness:null,deviceMode:null,deviceNight:null,deviceTouchCalibration:null,touchCalibrationWaiting:false,touchCalibrationBaseline:null,selectedPage:0,editingSettings:false,collapsed:new Set()};
...
function renderSettings(){
  ...
  const displayGrid=pageForm.firstElementChild.querySelector(".settings-grid");displayGrid.append(calibration);calibration.querySelectorAll("button").forEach(button=>button.onclick=async()=>{await loadLdrStatus();if(!Number.isFinite(state.ldrVoltage))return showToast("Todavía no hay una lectura del LDR.",true);const value=Math.round(state.ldrVoltage*100)/100;if(button.dataset.cal==="dark"){display.ldr_dark_voltage=value;displayGrid.querySelectorAll("input")[3].value=value}else{display.ldr_bright_voltage=value;displayGrid.querySelectorAll("input")[4].value=value}validate();showToast(`Calibración guardada en el editor: ${value.toFixed(2)} V`)});
...
function syncSettingsForm(){if(!state.editingSettings)return;const fields=[...pageForm.querySelectorAll("input,select")],{display,inactivity,night,sound}=state.ui.settings;if(fields.length<22)return;display.brightness=Number(fields[0].value);display.auto_brightness=fields[1].value==="yes";display.minimum_brightness=Number(fields[2].value);display.maximum_brightness=Number(fields[3].value);display.ldr_dark_voltage=Number(fields[4].value);display.ldr_bright_voltage=Number(fields[5].value);inactivity.timeout=Number(fields[6].value);state.ui.screensaver_timeout=inactivity.timeout;inactivity.mode=fields[7].value;inactivity.dim_brightness=Number(fields[8].value);night.enabled=fields[9].value==="yes";night.start=fields[10].value;night.end=fields[11].value;night.brightness=Number(fields[12].value);night.mode=fields[13].value;sound.enabled=fields[14].value==="yes";sound.touch=fields[15].value==="yes";sound.touch_volume=Number(fields[16].value);sound.navigation=fields[17].value==="yes";sound.navigation_volume=Number(fields[18].value);sound.notifications=fields[19].value==="yes";sound.notification_volume=Number(fields[20].value);sound.volume=sound.notification_volume;sound.mute_at_night=fields[21].value==="yes"}
...
function renderAll(){state.selectedPage=Math.max(0,Math.min(state.selectedPage,state.ui.pages.length-1));ensureSettings();renderPages();if(state.editingSettings){renderSettings()}else{stopLdrPolling();pageForm.classList.remove("settings-form");$("#duplicateButton").classList.remove("hidden");$("#deleteButton").classList.remove("hidden");$("#controlsHeading").classList.remove("hidden");controlList.classList.remove("hidden");renderPageForm();renderControls()}renderPreview();validate()}
...
saveButton.onclick=async()=>{syncSettingsForm();if(!(await validate()))return;saveButton.disabled=true;saveButton.textContent="Guardando…";try{const result=await api("/api/save",{method:"POST",body:JSON.stringify({ui:state.ui,backend_map:state.backendMap})}),problems=[];if(result.reload_error)problems.push(`pantalla: ${result.reload_error}`);if(result.ha_sync_error)problems.push(`Home Assistant: ${result.ha_sync_error}`);showToast(problems.length?`Guardado, pero falló ${problems.join(" · ")}`:`Guardado: pantalla y Home Assistant actualizados. Copia: ${result.backup}`,problems.length>0)}catch(error){showToast(error.message,true)}finally{saveButton.textContent="Guardar en Home Assistant";validate()}};
